...
public class ColorLerpControl : MonoBehaviour
{
    ...
    private Color maxColor = Color.red;   // Color for maximum EMG signal
    ...
    private static readonly int EMGValueProperty = Shader.PropertyToID("_EMG_Value");
    private static readonly int PulseSpeedProperty = Shader.PropertyToID("_Pulse_Speed");

    public float pulseSpeed = 1.0f; // Base speed for pulsing effect
    ...
    void Update()
    {
        ...
        if (currentIndex < emgData.Count)
        {
            ...
            // Pass normalized EMG value and pulse speed to the shader
            objRenderer.material.SetFloat(EMGValueProperty, normalizedValue);
            objRenderer.material.SetFloat(PulseSpeedProperty, pulseSpeed);
