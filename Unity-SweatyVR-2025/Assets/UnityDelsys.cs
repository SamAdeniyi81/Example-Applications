...
public class UnityDelsys : MonoBehaviour
{
   public EMGGraphController emgGraphController;
   ...
   //Paste key/license strings here
    ...
    public virtual void CollectionDataReady(object sender, ComponentDataReadyEventArgs e)
    {
        ...
        List<double> latestData = new List<double>();
        ...
        for (int k = 0; k < e.Data.Count(); k++)
        {
            ...
            for (int i = 0; i < e.Data[k].SensorData.Count(); i++)
            {
                ...
                {
// Assume we're only using the first channel for simplicity
                latestData.AddRange(e.Data[k].SensorData[i].ChannelData[j].Data);                
                }
            }
        }

    if (latestData.Count > 0 && emgGraphController != null)
    {
        // Assuming a fixed time interval between samples for demonstration
        float timeStep = 1f / 1000f; // Adjust according to your device's sampling rate
        float currentTime = 0f;

        foreach (var value in latestData)
        {
            emgGraphController.AddRealTimeDataPoint(currentTime, (float)value);
